...
/**
 * Asks for: 
 * - the name of a new language, and 
 * - source language, and 
 * - target language
 * 
...
public class TranslateColumnInputDialog extends InputDialog
{
     ...
     * The constructor
     */
    public TranslateColumnInputDialog(Shell parentShell, IProject project, String selectedColumn,
            String dialogTitle, String dialogMessage, String initialValue, IInputValidator validator)
    {
    ...
    protected Control createDialogArea(Composite parent)
    {
        ...
        Composite languagesComposite = (Composite) super.createDialogArea(parent);
        this.columnName = this.getText();

        ComboListener listener = new ComboListener();

        Group translationDetailsGroup = new Group(languagesComposite, SWT.SHADOW_ETCHED_OUT);
        translationDetailsGroup.setText(Messages.TranslationDialog_LanguageAreaLabel);
        translationDetailsGroup.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
        ...
        translatorsCombo =
                LanguagesUtil.createTranslatorsCombo((Composite) translationDetailsGroup);
        translatorsCombo.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 2, 1));
        ...
        String defaultLanguage =
                PreferencesManager.getInstance().getProjectPreferencesManager(this.project)
                        .getDefaultLanguage();

        fromCombo =
                LanguagesUtil.createLanguagesCombo(translationDetailsGroup, selectedLanguage,
                        defaultLanguage);
        fromCombo.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
        fromCombo.addSelectionListener(listener);

        LanguagesUtil.createImageStatus(translationDetailsGroup, ((selectedLanguage != null)
        ...
        toCombo = LanguagesUtil.createLanguagesCombo(translationDetailsGroup, null, null);
        ...
        automaticallyAddLangID.setText("Automatically add language ID to column name");
    ...
    /**
     * Selection Listener that will be responsible for monitoring
     * changes in combo selections (from language, to language and translator)
     */
