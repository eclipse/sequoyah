 ...
 * interface in order to get environment information from an
 * {@link IInstallableUnit} class instance's filter.
 * 
...
public class IUInstallationEnvironment implements IInstallationEnvironment {
    ...
    public IUInstallationEnvironment(String filter) {
        if (filter != null) {
            Matcher matcher = Pattern.compile("osgi[.]([^=])+[=]([^)])+")
                    .matcher(filter);
            while (matcher.find()) {
    ...
    /* (non-Javadoc)
     * @see org.eclipse.sequoyah.pulsar.internal.provisional.core.IInstallationEnvironment#getTargetOS()
     */
    ...
    /* (non-Javadoc)
     * @see org.eclipse.sequoyah.pulsar.internal.provisional.core.IInstallationEnvironment#getTargetOSArch()
     */
    ...
    /* (non-Javadoc)
     * @see org.eclipse.sequoyah.pulsar.internal.provisional.core.IInstallationEnvironment#getTargetWS()
     */
    ...
    private void processKeyValuePair(String[] pair) {
        ...
        } else if (key.equals("osgi.ws")) {
            this.ws = pair[0x01].trim();
        } else if (key.equals("osgi.arch")) {
