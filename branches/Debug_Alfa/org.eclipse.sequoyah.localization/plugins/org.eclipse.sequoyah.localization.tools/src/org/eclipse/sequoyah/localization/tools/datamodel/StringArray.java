...
public class StringArray
{
    ...
    /**
     * Set the value associated to the array in a specific position 
     * for the language represented by the localizationFile
     * 
     * @param value value associated to the key
     * @param position position where the new value will be added
     * @return the StringNode created
     */
    public StringNode addValue(String value, Integer position)
    {
        ...
        if (position == null)
        {
            ...
            if (this.values.size() > 0)
            {
                ...
                lastNumber = Integer.parseInt(((lastNode.getKey().split("_")[1])));
            }
            position = lastNumber + 1;
        }

        DecimalFormat formatter = new DecimalFormat("000");
        String virtualKey = this.key + "_" + formatter.format(position.intValue());
     ...
     * Remove string item
     * 
     * @param stringNode the StringNode to be removed
     */
    ...
    public static boolean isArrayItem(String key)
    {
        ...
        try
        {
            ...
            String itemPosition = key.substring(separatorIndex + 1, key.length());
            if ((arrayKey == null) || (arrayKey.equals("")))
            {
                result = false;
            }
            if ((itemPosition == null) || (itemPosition.equals("")) || (itemPosition.length() != 3))
            {
                result = false;
            }
    ...
    public static int findItemPosition(String key)
    {
        ...
        {
            int separatorIndex = key.lastIndexOf("_");
            position = Integer.parseInt(key.substring(separatorIndex + 1, key.length()));
        }
        catch (Exception e)
        {
            // do nothing
