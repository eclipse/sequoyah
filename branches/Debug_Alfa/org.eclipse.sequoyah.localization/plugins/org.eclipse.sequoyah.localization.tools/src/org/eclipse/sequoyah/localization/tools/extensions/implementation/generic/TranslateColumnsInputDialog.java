...
public class TranslateColumnsInputDialog extends Dialog
{
     ...
     * The constructor
     */
    public TranslateColumnsInputDialog(Shell parentShell, IProject project, String selectedColumn,
            String[] selectedCells, TableColumn[] columns, String dialogTitle)
    {
        super(parentShell);
    ...
    /**
     * Create origin area with information about selection and 
     * original language
     * 
     * @param parent parent composite
     * @param comboListener translator combo listener
            ...
            ILocalizationSchema schema)
    {
        ...
        labelFromText.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false, 2, 1));
        String selectedText = "";
        for (String text : this.selectedCells)
        {
            selectedText += text + "\n";
        }
        ...
        String defaultLanguage =
                PreferencesManager.getInstance().getProjectPreferencesManager(this.project)
                        .getDefaultLanguage();
        fromCombo =
                LanguagesUtil.createLanguagesCombo((Composite) languageFromGroup, selectedLanguage,
                        defaultLanguage);
        fromCombo.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
        fromCombo.addSelectionListener(comboListener);

        LanguagesUtil.createImageStatus(languageFromGroup, ((selectedLanguage != null)
                ? selectedLanguage : defaultLanguage));

    }

    /**
     * Create destination area with information about destination
     * columns and languages
     * 
     * @param parent parent composite
     * @param schema android localization schema
     */
        ...
        for (TableColumn column : this.columns)
        {
            if ((i > 0) && (!column.getText().equals(this.selectedColumn)))
            {
                ...
                DestinationColumn destColumn =
                        new DestinationColumn(checkbox, true, column.getText(), lang);
                ...
                Combo languagesCombo =
                        LanguagesUtil.createLanguagesCombo(languageToGroup, lang, null);
                ...
                languagesCombo.addSelectionListener(new SelectionAdapter()
                {
                    ...
                    public void widgetSelected(SelectionEvent e)
                    {
                        ((DestinationColumn) ((Combo) e.getSource()).getData())
                                .setLang(LanguagesUtil.getLanguageID(((Combo) e.getSource())
                                        .getText()));
                    }
     ...
     * changes in combo selections (from language and translator)
     * The combos for "to language" are differently managed
     */
    ...
    /**
     * This class represents a destination column, with
     * its language and name
     */
    public class DestinationColumn
    {
        ...
        public DestinationColumn(Button checkbox, boolean isSelected, String text, String lang)
        {
            super();
